...
import * as recognizer from "./recognizer-whisper.js";
...
export function init_game_start() {
  ...
  const saved = localStorage.getItem("whisperServer");
  if (saved) ipInput.value = saved;

  saveBtn.onclick = () => {
    const v = ipInput.value.trim();
    if (!v) return;
    localStorage.setItem("whisperServer", v);
    alert("✅ Whisper server saved");
  };
}
